...
- **대화 기록이 섞임** — 에이전트 작업 내역과 롤플레이 대화가 한 세션에섞임
- **시스템 프롬프트 충돌** — 에이전트용 SOUL.md와 롤플레이용 SOUL.md가 다름
- **모델 전환 번거로움** —매번 `/model`을수동 전환
...
2. **게이트웨이 포트**: 여러 프로필을 같은 서버에서실행할 때 포트 충돌에 주의
3. **API 키공유**: `.env`의 `NANOGPT_API_KEY`는 같은 키를 사용해도 됩니다 (NanoGPT 계정공유)
4. **메모리 분리**: 각 프로필의 대화 기록과 메모리는완전히 독립적입니다
